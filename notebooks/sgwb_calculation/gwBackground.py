...
class OmegaGW(object):
    ...
    def __init__(self,ref_mMin,ref_mMax,ref_zs,fmax,inspiralOnly,Mtots=[],qs=[],gridSize=(70,65)):
        ...
        self.ref_freqs = np.logspace(np.log10(1),np.log10(fmax),250)
...
class OmegaGW_BNS(OmegaGW):

    def __init__(self,ref_mMin,ref_mMax,ref_zs):
        super(OmegaGW_BNS,self).__init__(ref_mMin,ref_mMax,ref_zs,3000,True)

        # Compute probabilities, cut on masses
        probs = self.Mtots_2d**2./(1.+self.qs_2d)**2.
        probs[self.m1s_2d>self.ref_mMax] = 0
        probs[self.m2s_2d>self.ref_mMax] = 0
...
class OmegaGW_NSBH(OmegaGW):

    def __init__(self,ref_zs):
        super(OmegaGW_NSBH,self).__init__(1.,50.,ref_zs,3000,Mtots=np.logspace(np.log10(3.5),np.log10(50.),50),qs=np.linspace(1./50.,0.5,45))

        # Compute probabilities, cut on masses
        probs = self.Mtots_2d**2./(1.+self.qs_2d)**2.
        probs[self.m1s_2d<5.] = 0
        probs[self.m1s_2d>10.] = 0
        probs[self.m2s_2d<1.] = 0
        probs[self.m2s_2d>2.5] = 0
        probs /= np.sum(probs)

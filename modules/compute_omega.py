...
def calculate_total_rate(zs, p_dz):
    p_dz_centers = (p_dz[1:] + p_dz[:-1]) / 2.0
    return np.sum(np.diff(zs) * p_dz_centers)
...
def calculate_num_injections(T_obs, zs, p_dz):
    ...
    """
    total_rate = calculate_total_rate(zs, p_dz)
    return T_obs * total_rate
...
def calculate_omega_gridded(prior_dict, fref=25):
    ...
    for i in range(omg.m1s_2d.shape[0]):  # for each m1
        for j in range(omg.qs_2d.shape[1]):  # for each q
            prob = prior_dict.prob(
                {"mass_1": omg.m1s_2d[i][j], "mass_ratio": omg.qs_2d[i][j]}
            )  # calculate probability
            probs[i][j] = prob  # insert probability into probability array
    probs = probs * (omg.Mtots_2d / (1.0 + omg.qs_2d))  # multiply by the Jacobian

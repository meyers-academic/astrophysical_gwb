...
def R_SI(alpha, beta, z, zp, R0):
    ...
    return C(alpha, beta, zp) * R0 * (1 + z)**alpha / (1 + (
        (1 + z) / (1 + zp))**(alpha + beta))
...
def Hubble_rate(z, H0, omegaR, omegaM, omegak, omegaL):
    ...
    '''
    return H0 * (omegaR * (1 + z)**4 + omegaM * (1 + z)**3 + omegak *
                 (1 + z)**2 + omegaL)**(1 / 2)
...
def H(f, Mtot):
    ...
    eta = 0.25
    f_merge = (0.29740 * eta**2. + 0.044810 * eta + 0.095560) / (np.pi * Mtot *
                                                                 MsunToSec)
    f_ring = (0.59411 * eta**2. + 0.089794 * eta + 0.19111) / (np.pi * Mtot *
                                                               MsunToSec)
    f_cutoff = (0.84845 * eta**2. + 0.12828 * eta + 0.27299) / (np.pi * Mtot *
                                                                MsunToSec)
    sigma = (0.50801 * eta**2. + 0.077515 * eta + 0.022369) / (np.pi * Mtot *
                                                               MsunToSec)
    ...
    if f < f_merge:
        ...
        return 1 / (f_merge * f_ring**(4 / 3)) * (f / (1 +
                                                       ((f - f_ring) /
                                                        (sigma / 2))**2))**2
    else:
        return 0
